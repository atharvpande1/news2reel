...
The validator is async (it resolves DNS off the event loop), so the hermetic
tests drive it through asyncio.run. Patching socket.getaddrinfo still works:
loop.getaddrinfo delegates to it in an executor."""
...
async def test_validate_public_url_rejects_non_public_ip(monkeypatch, ip: str) -> None:
    ...
    with pytest.raises(UnsafeUrlError, match="non-public address"):
        await _validate_public_url("http://example.test/feed")


async def test_validate_public_url_accepts_public_ip(monkeypatch) -> None:
    monkeypatch.setattr(socket, "getaddrinfo", lambda *_a, **_kw: _addrinfo_for("93.184.216.34"))
    (await _validate_public_url("http://example.test/feed"))  # does not raise


async def test_validate_public_url_rejects_file_scheme() -> None:
    with pytest.raises(UnsafeUrlError, match="scheme"):
        await _validate_public_url("file:///etc/passwd")


async def test_validate_public_url_rejects_unresolvable_host(monkeypatch) -> None:
    def _raise(*_a, **_kw):
        raise socket.gaierror("nope")

    monkeypatch.setattr(socket, "getaddrinfo", _raise)
    with pytest.raises(UnsafeUrlError, match="could not resolve"):
        await _validate_public_url("http://nonexistent.invalid/feed")
...
async def test_fetch_url_returns_content_and_status(settings: Settings) -> None:
    with _local_server() as base_url:
        result = await fetch_url(f"{base_url}/ok", settings)
    assert result.status_code == 200
    assert result.content == b"ok"


async def test_fetch_url_follows_redirect_within_limit(settings: Settings) -> None:
    with _local_server() as base_url:
        result = await fetch_url(f"{base_url}/redirect-once", settings)
    assert result.status_code == 200
    assert result.final_url.endswith("/ok")


async def test_fetch_url_raises_on_redirect_loop(settings: Settings) -> None:
    settings.fetch_max_redirects = 2
    with _local_server() as base_url, pytest.raises(UnsafeUrlError, match="too many redirects"):
        await fetch_url(f"{base_url}/redirect-loop", settings)


async def test_fetch_url_raises_on_response_too_large(settings: Settings) -> None:
    settings.fetch_max_response_bytes = 10
    with _local_server() as base_url, pytest.raises(FetchTooLargeError):
        await fetch_url(f"{base_url}/big", settings)


async def test_fetch_url_wraps_connection_failure(settings: Settings) -> None:
    with pytest.raises(FetchTransportError):
        await fetch_url("http://127.0.0.1:1/rss", settings)  # nothing listens on port 1
